...
```
    1. Возможность приостановить / перезапустить игру
    2. Подсчёт времени сессии
    3. Отображение координаты игрока как игровой переменной
    4. Экран финиша со статистикой игры
    5. Смена ускорения свободного падения с набором высоты (имитация выхода в космос) (работает через смену параметра GravityScale у персонажа)
1. Так как проект изначально разрабатывался под Android, для "порта" на WebGL я сделал несколько косметических изменений, таких как смена ориентации на альбомную, смена управления с сенсорного на кнопки клавиатуры и адаптация канваса под десктоп. [OverviewAndroid.jpg]
